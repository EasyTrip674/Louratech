...
import { Unlink, Trash, Database, Info } from "lucide-react";
...
export default function DeleteClientStepFormModal({ ClientStepId, inPageProfile = false ,ClientStepName, authozise =false,clientProcedureId,procedureId}:
  ...
  const DeleteMutation = useMutation({
    mutationFn: async () => {
      try {
        const result = await doDeleteClientStep({
          ClientStepId,
          ClientStepName,
  ...
  return (
    <>
      ...
      <DeleteConfirmationModal
        ...
        itemName={ClientStepName}
        title="Supprimer cette étape pour ce client"
        isLoading={DeleteMutation.isPending}
        requireNameConfirmation={true}
        warningMessage={
          <p className="text-sm text-red-700 dark:text-red-300">
            <strong>Attention :</strong> Vous êtes sur le point de supprimer définitivement l'étape <span className="font-medium">{ClientStepName}</span> et toutes les données associées.
          </p>
        }
        warningSubMessage="Cette action est irréversible et entraînera la perte de toutes les données liées à cette étape."
      >
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                  Choisissez comment traiter les transactions liées à cette étape
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 ml-11">
              ...
              <div className="ml-2 text-sm">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                  {deleteTransactions
                    ? "Les transactions liées à cette étape seront définitivement supprimées."
                    : "Les transactions liées seront conservées mais leur référence à l'étape sera supprimée."
                  }
                </p>
              </div>
            </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                      Toutes les transactions, revenus et dépenses associées à cette étape seront définitivement supprimés. Cette action ne peut pas être annulée.
                    </p>
                  </div>
                </div>
              </div>
