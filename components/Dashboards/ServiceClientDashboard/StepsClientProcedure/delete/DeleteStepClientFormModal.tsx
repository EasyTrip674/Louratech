...
import { AlertTriangle, Trash, Database, Info, Unlink } from "lucide-react";
import { useMutation } from "@tanstack/react-query";
import SuccessModal from "@/components/alerts/SuccessModal";
import ErrorModal from "@/components/alerts/ErrorModal";
import { doDeleteClientStep } from "./clientStep.delete.action";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { deleteClientStepSchema } from "./stepClient.delete.shema";

// Infer the TypeScript type from the Zod schema
type ClientStepFormData = z.infer<typeof deleteClientStepSchema>;

export default function DeleteClientStepFormModal({ ClientStepId, inPageProfile = false ,ClientStepName, authozise =false,clientProcedureId,procedureId}:
   { ClientStepId:string, ClientStepName:string, authozise:boolean , inPageProfile?: boolean, clientProcedureId:string,procedureId:string }) {
  ...
  // Create a custom Zod schema that validates the name matches the ClientStep's name
  const enhancedDeleteClientStepSchema = deleteClientStepSchema.extend({
    ClientStepName: z.string()
      .min(1, "Veuillez saisir le nom de l'etape")
      .refine((val) => val.toLowerCase() === ClientStepName.toLowerCase(), {
  ...
  const {
    ...
    reset,
    watch,
  } = useForm<ClientStepFormData>({
    resolver: zodResolver(enhancedDeleteClientStepSchema),
    defaultValues: {
      ClientStepId,
      ClientStepName: "",
  ...
  const watchedname = watch("ClientStepName");
  const watchDeleteTransactions = watch("deleteTransactionAssocied");
  
  useEffect(() => {
    setNameMatch(watchedname?.toLowerCase() === ClientStepName.toLowerCase());
  }, [watchedname, ClientStepName]);

  const DeleteMutation = useMutation({
    mutationFn: async (data: ClientStepFormData) => {
      try {
        const result = await doDeleteClientStep(data);
        if (result?.data?.success) {
          closeModal();
          reset();
          successModal.openModal();
          router.refresh();
          router.push(`/services/gestion/procedures/${procedureId}/clients/${clientProcedureId}`)
          return result;
        } else {
          setServerError(result?.serverError || "Erreur lors de la suppression");
          errorModal.openModal();
          throw new Error("Failed to delete ClientStep");
        }
      } catch (error) {
  ...
  const onSubmit = (data: ClientStepFormData) => {
    if (nameMatch) {
  ...
  return (
    <>
      <SuccessModal 
        ...
        message='ClientStep supprimé avec succès'
        ...
        <div>
          ...
          <div className="p-4 mb-6 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/10 dark:border-red-900/30">
            <p className="text-sm text-red-700 dark:text-red-300">
              <strong>Attention :</strong> Vous êtes sur le point de supprimer définitivement le module pour ce client <span className="font-medium">{ClientStepName}</span> et toutes les procédures associées.
            </p>
          ...
          <div className="border-b border-gray-200 dark:border-gray-800 pb-6">
            <Label className="block mb-2">
              Pour confirmer la suppression, veuillez saisir le nom du module pour ce client <span className="font-medium italic text-amber-300">{ClientStepName}</span>
            </Label>
            <Input 
              {...register("ClientStepName")} 
              error={!!errors.ClientStepName} 
              hint={errors.ClientStepName?.message} 
              type="text" 
              placeholder={`Saisir "${ClientStepName}"`}
              className={`${nameMatch ? "border-green-500 dark:border-green-500" : ""}`}
            />
            
            {nameMatch && !errors.ClientStepName && (
