"use client";
import React, { useRef, useEffect, useCallback, useLayoutEffect, forwardRef } from "react";
...
interface ModalProps {
  ...
  labelledById?: string; // Pour aria-labelledby
}
...
export const Modal = React.memo(forwardRef<HTMLDivElement, ModalProps>(
  ({
    ...
    // Focus trap et focus auto
    useLayoutEffect(() => {
      if (!isOpen) return;
      const node = modalRef.current;
      let cleanup: (() => void) | undefined;
      if (node) {
        // Focus sur le premier focusable
        setTimeout(() => {
    ...
    const contentClasses = isFullscreen
      ? "w-full h-full"
      : "relative w-full max-w-lg sm:max-w-xl md:max-w-2xl rounded-3xl bg-white dark:bg-gray-900 shadow-lg animate-modal-in";
    ...
    return (
      ...
      >
        {!isFullscreen && (
          <div
            className="fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px] transition-opacity duration-200 animate-fade-in"
