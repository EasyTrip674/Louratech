...
export default function DeleteProcedureFormModal({ procedureId,countClient, inPageProfile = false ,procedureName, authozise =false,}: { procedureId:string, procedureName:string, authozise:boolean , inPageProfile?: boolean, countClient:number }) {
  ...
  // Create a custom Zod schema that validates the name matches the Procedure's name
  const enhancedDeleteProcedureSchema = deleteProcedureSchema.extend({
    procedureName: z.string()
      .min(1, "Veuillez saisir le nom du service")
      .refine((val) => val.toLowerCase() === procedureName.toLowerCase(), {
        message: "Le nom saisi ne correspond pas au nom du service",
      }),
  });
  ...
  const {
    ...
    resolver: zodResolver(enhancedDeleteProcedureSchema),
    defaultValues: {
      procedureId,
      procedureName: "",
  ...
  const watchedname = watch("procedureName");
  const watchDeleteTransactions = watch("deleteTransactionAssocied");
  
  useEffect(() => {
    setNameMatch(watchedname?.toLowerCase() === procedureName.toLowerCase());
  }, [watchedname, procedureName]);
      ...
      >
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h4 className="text-lg font-medium text-red-700 dark:text-red-400">Supprimer ce service</h4>
          </div>
          ...
          <div className="p-4 mb-6 border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/10 dark:border-red-900/30">
            ...
            <p className="mt-2 text-sm text-red-700 dark:text-red-300">
              Cette action est irréversible et entraînera la perte de toutes les données liées à ce service.
            </p>
          </div>

          <div className="border-b border-gray-200 dark:border-gray-800 pb-6">
            <Label className="block mb-2">
              Pour confirmer la suppression, veuillez saisir le nom du service <span className="font-medium italic text-amber-300">{procedureName}</span>
            </Label>
            <Input 
              {...register("procedureName")} 
              error={!!errors.procedureName} 
              hint={errors.procedureName?.message} 
              type="text" 
              placeholder={`Saisir "${procedureName}"`}
              className={`${nameMatch ? "border-green-500 dark:border-green-500" : ""}`}
            />
            
            {nameMatch && !errors.procedureName && (
